"use client";
import { useEffect, useState } from 'react'
import styles from './ProjectModal.module.css'

const sections = ['Overview', 'Problem & Solution', 'Tech Stack', 'Results']

export default function ProjectModal({ project, onClose }) {
  const [activeSection, setActiveSection] = useState('Overview')

  useEffect(() => {
    document.body.style.overflow = 'hidden'
    const onKey = (e) => { if (e.key === 'Escape') onClose() }
    window.addEventListener('keydown', onKey)
    return () => {
      document.body.style.overflow = ''
      window.removeEventListener('keydown', onKey)
    }
  }, [onClose])

  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-content">
        <div className={styles.termBar}>
          <div className={styles.termDots}>
            <span className={styles.termDot} style={{ background: '#f85149' }} />
            <span className={styles.termDot} style={{ background: '#d29922' }} />
            <span className={styles.termDot} style={{ background: '#3fb950' }} />
          </div>
          <span className={styles.termFile}>~/projects/{project.id}.md</span>
        </div>

        <div className={styles.modalHeader}>
          <div className={styles.headerLeft}>
            <span className={styles.typeTag}>{project.type}</span>
            <h2 className={styles.modalTitle}>{project.name}</h2>
            <p className={styles.modalTagline}>{project.tagline}</p>
          </div>
          <div className={styles.headerRight}>
            <div className={styles.modalLinks}>
              <a href={project.github} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/>
                </svg>
                GitHub
              </a>
              {project.demo && (
                <a href={project.demo} target="_blank" rel="noopener noreferrer" className="btn btn-primary btn-sm">
                  Live Demo
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/>
                  </svg>
                </a>
              )}
            </div>
            <button className={styles.closeBtn} onClick={onClose} aria-label="Close">
              :q
            </button>
          </div>
        </div>

        <div className={styles.sectionBar}>
          {sections.map(s => (
            <button
              key={s}
              className={`${styles.sectionBtn} ${activeSection === s ? styles.sectionActive : ''}`}
              onClick={() => setActiveSection(s)}
            >
              ## {s}
            </button>
          ))}
        </div>

        <div className={styles.modalBody}>
          {activeSection === 'Overview' && (
            <div className={styles.sectionContent}>
              <p className={styles.bodyText}>{project.description}</p>
              <div className={styles.archBlock}>
                <p className={styles.archOutput}>{project.architecture}</p>
              </div>
            </div>
          )}

          {activeSection === 'Problem & Solution' && (
            <div className={styles.sectionContent}>
              <div className={styles.psGrid}>
                <div className={styles.psBlock}>
                  <div className={styles.psLabel}>
                    <span className={styles.psIcon} style={{ background: 'rgba(248,81,73,0.1)', color: '#f85149' }}>⚠</span>
                    The Problem
                  </div>
                  <p className={styles.bodyText}>{project.problem}</p>
                </div>
                <div className={styles.psBlock}>
                  <div className={styles.psLabel}>
                    <span className={styles.psIcon} style={{ background: 'rgba(63,185,80,0.1)', color: '#3fb950' }}>✓</span>
                    The Solution
                  </div>
                  <p className={styles.bodyText}>{project.solution}</p>
                </div>
              </div>
              {project.challenges && (
                <div className={styles.archBlock} style={{ marginTop: 24 }}>
                  <p className={styles.archLabel}>Key Challenges</p>
                  <ul className={styles.challengeList}>
                    {project.challenges.map((c, i) => (
                      <li key={i} className={styles.challengeItem}>
                        <span className={styles.challengeDot} />
                        <span className={styles.bodyText}>{c}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              )}
            </div>
          )}

          {activeSection === 'Tech Stack' && (
            <div className={styles.sectionContent}>
              <div className={styles.depBlock}>
                <p className={styles.depLabel}>Dependencies:</p>
                <div className={styles.depList}>
                  {project.tech.map((t, i) => (
                    <div key={t} className={styles.depItem}>
                      <span className={styles.depPrefix}>
                        {i === project.tech.length - 1 ? '└── ' : '├── '}
                      </span>
                      <span className={styles.depName}>{t}</span>
                    </div>
                  ))}
                </div>
              </div>
            </div>
          )}

          {activeSection === 'Results' && (
            <div className={styles.sectionContent}>
              <div className={styles.resultsList}>
                {project.results.map((r, i) => (
                  <div key={i} className={styles.resultItem}>
                    <span className={styles.resultCheck}>[✓]</span>
                    <span className={styles.bodyText}>{r}</span>
                  </div>
                ))}
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  )
}
